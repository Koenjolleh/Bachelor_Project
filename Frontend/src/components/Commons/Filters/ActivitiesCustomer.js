...
class ActivitiesCustomer extends Component {
    state = {
        options: [],
        service: undefined
    };
    ...
    componentDidUpdate(prevProps,prevState){
        ...
        if(options !== prevState.options){
            if(options.length > 0){
                ...
                if(checked.length > 0){
                    this.props.selectActivitiesDay(checked, service, 2);
                } else if(checked.length === 0){
                    this.props.selectActivitiesDay(checked, service, 3);
                }
            }
        }
    }
...
export default withStyles(styles)(ActivitiesCustomer);