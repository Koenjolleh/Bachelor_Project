...
class MetricsCards2 extends Component {
    state = {
        data: [],
        openModal: false,
        cardSelected: null,
        potentialGain: [
            'Daily get 5 more people from popular zone to convert to sales. With average transaction of 300DKK, potential gain per month 45000DKK.',
            'If we target 5 pct. of the commuters, it represents 166 new weekly customers.  If 10% convert to sales with an average transaction of 300kr/customer, monthly potential increase in revenue is DKK 19.200.',
            'We target 1% of 16.400 passers-bys, converting 30% to customers will, with DKK 300 average transaction, mean an daily extra topline potential of DKK60.000.',
            '', '', '', '', '', '', '', ''],
...
//   isSuccess: null, learningsDisable: false },
    ...
    handleCard = (event) => {
        let idCardSelected = parseInt(event.target.id,10);
            this.setState((prevState) => ({
                openModal: !prevState.openModal,
                cardSelected: idCardSelected
            }), () => console.log("cardSelected: ", this.state.cardSelected));
    }

    render(){
        const { classes } = this.props;
        const { data, youraction, myaction, isDisableYourAction, isDisableMyAction, myNewAction, myNewActionDisable, 
                listTime, timeSelected, header, isSuccess, openModal, potentialGain, potentialGainDisable,
                learnings, learningsDisable, incompleteInsights, cardSelected,
                // newInsight 
            } = this.state;
            ...
            <Fragment>
                        ...
                        <Fade in={openModal}>
                            <div className={classes.modalPaper}>
                                ...
                                <Table className={classes.table}>
                                    ...
                                    <TableBody>
                                    {
                                        data.filter(f => f.id === cardSelected).map(row => (
                                            <Fragment>
                ...
                <Paper className={classes.root}>
                {
                    data.map(d => {
                        return(
                            <Fragment>
                                <Card key={d.id} id={d.id} className={classes.card} onClick={this.handleCard}>
                                    <CardActionArea id={d.id} className={classes.cardActionArea}>
                                        <CardMedia
                                            className={classes.cardMedia}
                                            component="img"
                                            alt="Insights and Actions"
                                            image={require(`../../../assets/${d.image}`)}
                                            title="Insights and Actions"
                                            id={d.id}
                                        />
                                        <CardContent id={d.id} className={classes.cardContent}>
                                            <Typography id={d.id} gutterBottom variant="h4" component="h2" className={classes.typography}>
                                                {d.insightCard}
                                            </Typography>
                                            <Typography id={d.id} variant="body2" component="p" className={classes.typography}>
                                                {d.insight}
                                            </Typography>
                                            <br />
                                            <Typography id={d.id} gutterBottom variant="h4" component="h2" className={classes.typography}>
                                ...
                                {/* <Fragment>
                                {
                                    (d.id + 1) % 2 === 0 ? <Fragment><br /><br /></Fragment> : null
                                }
                                </Fragment> */}
                            </Fragment>
                        )
...
MetricsCards2.propTypes = {
  classes: PropTypes.object.isRequired,
  data: PropTypes.array.isRequired
};

export default withStyles(styles)(withRouter(MetricsCards2));