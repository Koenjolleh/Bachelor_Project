...
function getDataByDayByActivity(idLocation, selectedDay, activityDay, service, returningCustomer, defaultDataset) {
    ...
    return async (dispatch, getState) => {
        ...
        try {
            ...
            const outside = { outside: data_byday_byactivity.data.data, inside: getState().byactivities.dataByDayByActivity.inside };
            dispatch({ type: GET_DATA_BY_DAY_BY_ACTIVITY_SUCCESS, payload: outside });
...
function getDataByTypeDaysByActivity(idLocation, typeDay, activityDay, service, returningCustomer, defaultDataset) {
    ...
    return async (dispatch, getState) => {
        ...
        try {
            ...
            const outside = { outside: data_typedays_byactivity.data.data, inside: getState().byactivities.dataByTypeDaysByActivity.inside };
            dispatch({ type: GET_DATA_BY_TYPEDAYS_BY_ACTIVITY_SUCCESS, payload: outside });
...
function getDataFullDaysByActivity(idLocation, activityDay, service, returningCustomer, defaultDataset) {
    ...
    return async (dispatch, getState) => {
        ...
        try {
            ...
            const outside = { outside: data_fulldays_byactivity.data.data, inside: getState().byactivities.dataFullDaysByActivity.inside };
            dispatch({ type: GET_DATA_FULL_DAYS_BY_ACTIVITY_SUCCESS, payload: outside });
...
/****** getByActivitiesData **********/
export const getByActivitiesOutsideData = (idLocation, day, activities, service, returningCustomer, defaultDataset) => {
    const id_location = idLocation;
    const id_day = day;
