...
function getDataByDayByPeriodByActivity(idLocation, selectedDay, periodsDay, activityDay, service, returningCustomer, defaultDataset) {
    ...
    return async (dispatch, getState) => {
        ...
        try {
            ...
            const outside = { outside: data_day_period_activity.data.data, inside: getState().byperiodsbyactivities.dataByDayByPeriodByActivity.inside };
            dispatch({ type: GET_DATA_DAYS_PERIOD_ACTIVITY_SUCCESS, payload: outside });
...
function getDataByTypeDaysByPeriodByActivity(idLocation, typeDay, periodsDay, activityDay, service, returningCustomer, defaultDataset) {
    ...
    return async (dispatch, getState) => {
        ...
        try {
            ...
            const outside = { outside: data_typedays_period_activity.data.data, inside: getState().byperiodsbyactivities.dataByTypeDaysByPeriodByActivity.inside };
            dispatch({ type: GET_DATA_TYPEDAYS_PERIOD_ACTIVITY_SUCCESS, payload: outside });
...
function getDataByFullDaysByPeriodByActivity(idLocation, periodsDay, activityDay, service, returningCustomer, defaultDataset) {
    ...
    return async (dispatch, getState) => {
        ...
        try {
            ...
            const outside = { outside: data_fulldays_period_activity.data.data, inside: getState().byperiodsbyactivities.dataByFullDaysByPeriodByActivity.inside };
            dispatch({ type: GET_DATA_FULLDAYS_PERIOD_ACTIVITY_SUCCESS, payload: outside });
...
export const getByPeriodsByActivitiesOutsideData = (idLocation, day, periods, activities, service, returningCustomer, defaultDataset) => {
    const id_location = idLocation;
    const id_day = day;
